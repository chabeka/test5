...
public abstract class AbstractBaseTestCase {
   ...
   public static final String SERVICE_URL = "http://cer69-ds4int:8080/dfce-webapp/toolkit/";
   ...
   protected static Document storeDoc(Document document, File newDoc, boolean expectStore) {
      Document stored;
      FileInputStream in = null;
      try {
         in = new FileInputStream(newDoc);
         String type = newDoc.getName().substring(newDoc.getName().lastIndexOf(".") + 1)
               .toUpperCase();
         document.setType(type);

         stored = ServiceProvider.getStoreService().storeDocument(document, in);
