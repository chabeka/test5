...
public class RechercheTestService {
   ...
   private RechercheResponse appelWsOpRecherche(
      ...
      try 
      {
         ...
         Recherche paramsService = SaeServiceObjectFactory.buildRechercheRequest(
         ...
         log.appendLogLn("Détails de la réponse obtenue de l'opération \"recherche\" :");
         SaeServiceLogUtils.logResultatRecherche(
               log, 
    ...
    */
   public final void appelWsOpRechercheTestLibre(
         String urlServiceWeb,
         RechercheFormulaire formulaire) {
    ...
    * @param urlServiceWeb l'URL du service web SaeService
    * @param formulaire le formulaire
    * @param nbResultatsAttendus le nombre de résultats attendus, ou null si pas de vérif
    * @param flagResultatsTronquesAttendu le flag attendu, ou null si pas de verif
    * @return la réponse de l'opération "recherche", ou null si une exception s'est produite
    */
   public final RechercheResponse appelWsOpRechercheReponseCorrecteAttendue(
      ...
      // Appel de la méthode "générique" de test
      RechercheResponse response = appelWsOpRecherche(
            urlServiceWeb,
            ViUtils.FIC_VI_OK,
            formulaire,
            testAvecReponse,
      ...
      // On vérifie le résultat obtenu (si le test n'a pas échoué dans les étapes préalables)
      ResultatTest resultatTest = formulaire.getResultats();
      if (!TestStatusEnum.Echec.equals(resultatTest.getStatus())) {
         ...
         if (response==null) {
            ...
            ResultatTestLog log = resultatTest.getLog();
            log.appendLogLn("On s'attendait à une réponse sans erreur du service web, hors on a obtenu une réponse null");
            
         } else {
            
            // Vérification de la réponse par rapport aux paramètres envoyés au service web
            wsVerifieRetour(
                  resultatTest,
                  response,
                  nbResultatsAttendus,
   ...
   private void wsVerifieRetour(
      ...
      if ((response!=null) && (response.getRechercheResponse()!=null)) {
         ...
         boolean resultatTronque = response.getRechercheResponse().getResultatTronque();
         ResultatRechercheType[] resultats = null;
         if (response.getRechercheResponse().getResultats()!=null) {
            resultats = response.getRechercheResponse().getResultats().getResultat();
         }
         ...
         if (  (nbResultatsAttendus!=null) && 
            ...
            log.appendLog(" résultat(s) de recherche, alors que l'on en a obtenu ");
         ...
         if (  (flagResultatsTronquesAttendu!=null) && 
               (resultatTronque!=flagResultatsTronquesAttendu.booleanValue())) {
         
            resultatTest.setStatus(TestStatusEnum.Echec);
            
            log.appendLogNewLine();
            log.appendLog("Erreur : on s'attendait à obtenir le flag de résultat tronqué à ");
         ...
         // Soit :
         //  - on vérifie que l'on a obtenu, par défaut, les métadonnées dites "consultées par défaut"
         //  - ou on vérifie que l'on a obtenu les métadonnées demandées
         if (resultats!=null) {
            if (CollectionUtils.isNotEmpty(codesMetaAttendues)) {
               
               wsVerifieRetourMetaDemandees(resultatTest,codesMetaAttendues,resultats);
   ...
   private void wsVerifieRetourMetaDemandees(
         ResultatTest resultatTest,
         CodeMetadonneeList codesMetaAttendues,
      ...
      log.appendLogNewLine();
      testMetaService.areMetadonneesConsultables(
            codesMetaAttendues,
            resultatTest);
      ...
      List<MetadonneeDefinition> metadonneesDefinitions =
         referentielMetadonneesService.construitListeMetadonnee(codesMetaAttendues);
      
      // Puis on boucle sur les résultats de la recherche, afin de contrôler chaque résultat
      String messageErreur1;
      String messageErreur2;
      String messageErreurAll;
      for(int i=0;i<resultats.length;i++) {
         
         // 1) Vérifie que les métadonnnées retournées font bien partie de la liste demandées
         
         messageErreur1 = testMetaService.verifieMetasUniquementDansListeAutorisee(
               resultats[i].getMetadonnees(), 
               metadonneesDefinitions);
         
         // 2) Vérifie que toutes les métadonnées demandées font bien partie de la liste
         ...
         // Bilan des erreurs
         messageErreurAll = TestUtils.concatMessagesErreurs(messageErreur1, messageErreur2);
         if (StringUtils.isNotBlank(messageErreurAll)) {
            
            resultatTest.setStatus(TestStatusEnum.Echec);
            
            log.appendLogNewLine();
            log.appendLogLn("Erreur sur les métadonnées du résultat #" + (i+1));
   ...
   private void wsVerifieRetourMetaParDefaut(
         ResultatTest resultatTest,
         ResultatRechercheType[] resultats) {
      ...
      for(int i=0;i<resultats.length;i++) {
         ...
         messageErreur = testMetaService.verifieMetadonneesConsulteeParDefaut(
               resultats[i].getMetadonnees());
    ...
    * @param urlServiceWeb l'URL du service web SaeService
    * @param formulaire le formulaire
    * @param ficVi le fichier de VI à utiliser parmi les constantes des ViUtils
    * @param idSoapFaultAttendu l'identifiant de la SoapFault attendu dans le référentiel des SoapFault
    * @param argsMsgSoapFault les arguments pour le String.format du message de la SoapFault attendue
    */
   public final void appelWsOpRechercheSoapFault(
         String urlServiceWeb,
         RechercheFormulaire formulaire,
         String ficVi,
         String idSoapFaultAttendu,
      ...
      WsTestListener listener = new WsTestListenerImplSoapFault(
            faultAttendue,
            argsMsgSoapFault);
   ...
   /**
    * Vérifie qu'un résultat de recherche contienne bien les métadonnées fournies en
    * paramètres, avec les valeurs fournies.
    * 
    * @param resultatRecherche l'objet "résultat de recherche"
    * @param numeroResultatRecherche le numéro du résultat de recherche, pour le log
    * @param resultatTest l'objet contenant le résultat du test en cours
    * @param valeursAttendues la liste des métadonnées attendues (codes + valeurs)
    */
   public final void verifieResultatRecherche(
         ResultatRechercheType resultatRecherche,
         String numeroResultatRecherche,
         ResultatTest resultatTest,
      ...
      MetadonneeValeurList metadonnees = 
         SaeServiceObjectExtractor.extraitMetadonnees(resultatRecherche);
      
      for (MetadonneeValeur valeurAttendue: valeursAttendues) {
         verifiePresenceEtValeur(
               resultatTest,
               metadonnees,
               numeroResultatRecherche,
               sbErreurs,
               valeurAttendue.getCode(),
   ...
   private void verifiePresenceEtValeur(
         ResultatTest resultatTest,
         MetadonneeValeurList metadonnees,
         String numeroResultatRecherche,
         StringBuffer sbErreurs,
         String code,
