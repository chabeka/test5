...
public class ConsultationTestService {
   ...
   private ConsultationResponse appelWsOpConsultation(
      ...
      try 
      {
         ...
         Consultation paramsService = SaeServiceObjectFactory.buildConsultationRequest(
               formulaire.getIdArchivage());
         ...
         log.appendLogLn("Détails de la réponse obtenue de l'opération \"consultation\" :");
   ...
   /**
    * Test libre de l'appel à l'opération "consultation" du service web SaeService.<br>
    * 
    * @param urlServiceWeb l'URL du service web SaeService
    * @param formulaire le formulaire
    */
   public final void appelWsOpConsultationTestLibre(
         String urlServiceWeb,
         ConsultationFormulaire formulaire) {
    ...
    */
   public final void appelWsOpConsultationSoapFault(
         String urlServiceWeb,
         ConsultationFormulaire formulaire,
         String ficVi,
      ...
      WsTestListener testAuth = new WsTestListenerImplSoapFault(
            faultAttendue,
            argsMsgSoapFault);
   ...
   private void wsVerifieRetour(
      ...
      // Récupère l'objet numérique dans le bon type, pour des besoins techniques
      ObjetNumeriqueConsultationType objNum = 
         response.getConsultationResponse().getObjetNumerique();
      
      // Fait le point sur ce qu'on a demandé par rapport à ce qu'on a récupéré
      boolean urlConsultDemandee = false;
      boolean urlConsultRecuperee = SaeServiceTypeUtils.isObjetNumeriqueUrlConsultDirecte(objNum); 
      boolean contenuBase64recupere = SaeServiceTypeUtils.isObjetNumeriqueContenuBase64(objNum);
      
      // Vérification que ce qu'on a reçu correspond à ce qu'on a demandé 
      boolean testKoTemp = wsIsRecuConformeAdemande(
            urlConsultDemandee,
            urlConsultRecuperee,
            contenuBase64recupere,
            log);
      if (testKoTemp) {
         testKo = true;
      }
         
      // Vérification du contenu (SHA-1) si fourni en paramètre + contenu base 64
      testKoTemp = wsVerifSha1(
            urlConsultDemandee,
            contenuBase64recupere,
            sha1attendu,
            log,
      ...
      String erreur = testsMetasService.verifieMetadonneesConsulteeParDefaut(
            response.getConsultationResponse().getMetadonnees());
   ...
   private boolean wsIsRecuConformeAdemande(
         boolean urlConsultDemandee,
         boolean urlConsultRecuperee,
         boolean contenuBase64recupere,
         ResultatTestLog log) {
      ...
      if (urlConsultDemandee) {
         ...
         if (!urlConsultRecuperee) {
            ...
            testKo = true;
            log.appendLogLn("On a demandé au service web de consultation une URL de consultation directe, hors on ne l'a pas reçu");
         }
         
         if (contenuBase64recupere) {
            // Pourtant, on a reçu un contenu en base64
            testKo = true;
            log.appendLogLn("On a demandé au service web de consultation une URL de consultation directe, pourtant on a reçu un contenu en base64");
         }
         ...
         if (urlConsultRecuperee) {
            ...
            testKo = true;
            log.appendLogLn("On n'a PAS demandé au service web de recevoir une URL de consultation directe, pourtant on en a reçu une");
         }
         
         if (!contenuBase64recupere) {
            // Pourtant, on a reçu un contenu en base64 
            testKo = true;
            log.appendLogLn("On n'a PAS demandé au service web de recevoir une URL de consultation directe, pourtant on n'a pas reçu de contenu en base64");
         }
   ...
   private boolean wsVerifSha1(
      ...
      if (  (!urlConsultDemandee) && 
         ...
         // Récupère le contenu du document renvoyé dans la réponse de consultation
         DataHandler contenu = 
            objNum.getObjetNumeriqueConsultationTypeChoice_type0().getContenu();
         ...
         if (!StringUtils.equalsIgnoreCase(sha1obtenu, sha1attendu)) {
            testKo = true;
            log.appendLogLn(
                  "Le SHA-1 du contenu de l'archive renvoyée (" + 
                  sha1obtenu + 
    ...
    * @param urlServiceWeb l'URL du service web SaeService
    * @param formulaire le formulaire
    * @param sha1attendu le SHA-1 attendu, ou null si pas de SHA-1
    * @return la réponse de l'opération "consultation", ou null si une exception s'est produite
    */
   public final ConsultationResponse appelWsOpConsultationReponseCorrecteAttendue(
      ...
      // Appel de la méthode "générique" de test
      ConsultationResponse response = appelWsOpConsultation(
            urlServiceWeb,
            ViUtils.FIC_VI_OK,
            formulaire,
            testAvecReponse);
