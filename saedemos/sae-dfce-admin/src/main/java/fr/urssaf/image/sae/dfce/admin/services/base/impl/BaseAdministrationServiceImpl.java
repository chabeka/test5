...
public class BaseAdministrationServiceImpl extends AbstractService implements
   ...
   public final void createBase(final DataBaseModel dataBaseModel,
      ...
      try {
         ...
         throw new BaseAdministrationServiceEx(MessageHandler.getMessage(
               MessageHandler.getMessage("database.already.exists",
                     dataBaseModel.getBase().getBaseId()), MessageHandler
                     .getMessage("database.already.exist.impact"),
               MessageHandler.getMessage("database.already.exist.action")),
               objAlreadyExistsEx);
      }
   ...
   public final void deleteBase(final DataBaseModel dataBaseModel) {
      ...
      if (base != null) {
         ...
         getServiceProvider().getBaseAdministrationService().deleteBase(base);
   ...
   public final void openConnection() throws ConnectionServiceEx {
      ...
      try {
         getServiceProvider().connect(connectionParameter.getUser().getLogin(),
      ...
      for (Rule documentRule : lifeCycles.getRules()) {
         try {
            ...
            LOGGER.info(MessageHandler.getMessage(
                  "lifeCycleRule.already.exist", documentRule.getRndCode()));
         }
      }
   }

   @Override
   public void updateAllIndexesUsage(DataBaseModel dataBaseModel) {
      LOGGER.info(MessageHandler.getMessage("database.retrieve"));
      // Instantiation d'une base
      final Base base = getServiceProvider().getBaseAdministrationService()
            .getBase(dataBaseModel.getBase().getBaseId());
      if (base != null) {
         LOGGER.info(MessageHandler.getMessage("update.start"));
         getServiceProvider().getStorageAdministrationService()
               .updateAllIndexesUsageCount();
         LOGGER.info(MessageHandler.getMessage("update.end"));
      }
   }
}
