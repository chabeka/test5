...
public class JaxbSommaireUnserializer implements SommaireUnserializerStrategy {
   private static final Logger LOGGER = LoggerFactory
         .getLogger(JaxbSommaireUnserializer.class);
   @Autowired
   ...
   private static final String SEPARATOR_FILE = System
   ...
   public final Sommaire unserializeSommaire(File fileXml)
         throws EcdeGeneralException {
      String prefixeTrc = "unserializeSommaire()";
      ...
      try {
         ...
         if (MessageRessourcesUtils.recupererMessage("invalid.batchmode.error",
               null).equals(except.getMessage())) {
            ...
            try {
               ...
               LOGGER
                     .error(
                           "Impossible de creer le fichier témoin fin_traitement.flag.",
                           e);
            }
   ...
   private Sommaire mapToUntypedDoc(SommaireType sommaireType,
      ...
      String prefixeTrc = "mapToUntypedDoc()";
      LOGGER.debug("{} - Début", prefixeTrc);
      LOGGER
            .debug(
                  "{} - Début du mapping de l'objet Jaxb représentant le sommaire.xml vers un objet métier Sommaire",
                  prefixeTrc);
      ...
      List<DocumentType> listDocsType = sommaireType.getDocuments()
            .getDocument();
      ...
      for (DocumentType docType : listDocsType) {
         ...
         chemEtNomFile = StringUtils
               .replace(chemEtNomFile, "/", SEPARATOR_FILE);
         chemEtNomFile = StringUtils.replace(chemEtNomFile, "\\",
               SEPARATOR_FILE);
         try {
            ...
            List<MetadonneeType> metaDataType = docType.getMetadonnees()
                  .getMetadonnee();
            ...
            erreurType.setLibelle(MessageRessourcesUtils.recupererMessage(
                  "ecde.invaliddoc.error", chemEtNomFile).concat(" Détails : ")
            ...
            throw new EcdeInvalidDocumentException(MessageRessourcesUtils
                  .recupererMessage("ecde.invaliddoc.error", chemin).concat(
                        " Détails : ").concat(except.getMessage()), except);
         }
      }
   ...
   private void exist(UntypedDocument untypedDoc, String chemin,
         String cheminEtNomFichier) throws IOException {
      String prefixeTrc = "exist()";
      LOGGER.debug("{} - Début", prefixeTrc);
      LOGGER.debug(
            "{} - Début de la vérification de l'existence du fichier : {} ",
            prefixeTrc, chemin);
      ...
      if (fichier.isFile()) {
         ...
         // ici on ne charge plus le contenu du document.
         // Le chargement est fait ultérieurement dans la couche storage.
         // untypedDoc
   ...
   private ResultatsType affectResultatsOnError(File fileXml) {
      ...
      erreurType
            .setLibelle(MessageRessourcesUtils.recupererMessage(
                  "som.invalid.error", null).concat(" Détails : ").concat(
                  MessageRessourcesUtils.recupererMessage("nodoc.insert.ecde",
                        null)));
   ...
   private ResultatsType affectResultatsInvalidDoc(SommaireType sommaireType,
      ...
      resultats.setInitialDocumentsCount(sommaireType.getDocuments()
            .getDocument().size());
      resultats.setInitialVirtualDocumentsCount(sommaireType
            .getDocumentsVirtuels().getDocumentVirtuel().size());
      ...
      for (DocumentType documentType : docsType) {
         ...
         if (chemin.equals(ecdeDirectory.concat(SEPARATOR_FILE).concat(
               DOCUMENTS).concat(SEPARATOR_FILE).concat(
               documentType.getObjetNumerique().getCheminEtNomDuFichier()))) {
