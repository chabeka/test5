...
public class JaxbSommaireUnserializer implements SommaireUnserializerStrategy {
	...
	public final Sommaire unserializeSommaire(File fileXml)
		...
		try {
			...
			if (MessageRessourcesUtils.recupererMessage(
					"invalid.batchmode.error", null)
					.equals(except.getMessage())) {
				...
				throw new EcdeInvalidBatchModeException(
	...
	private ResultatsType affectResultatsOnError(File fileXml) {
		...
		erreurType.setLibelle(MessageRessourcesUtils
				.recupererMessage("som.invalid.error", null)
				.concat(" Détails : ")
				.concat(MessageRessourcesUtils.recupererMessage(
						"nodoc.insert.ecde", null)));
	...
	private ResultatsType affectResultatsInvalidDoc(SommaireType sommaireType,
		...
		for (DocumentType documentType : docsType) {
			...
			if (chemin.equals(ecdeDirectory
					...
					.concat(DOCUMENTS)
					.concat(SEPARATOR_FILE)
					.concat(documentType.getObjetNumerique()
							.getCheminEtNomDuFichier()))) {
