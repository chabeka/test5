...
public class RNDReferenceDAOImpl implements RNDReferenceDAO {
	...
	public final Map<String, TypeDocument> getAllRndCodes()
		...
		Map<String, TypeDocument> rndReference = null;
		try {
			rndReference = xmlDataService.rndReferenceReader(referentiel
					.getInputStream());

		} catch (IOException e) {
			serviceDispatcher.dispatch(new ReferentialRndException(
					MetadataMessageHandler.getMessage("rcnd.file.notfound",
							referentiel.getFilename()), e));
		}
		return rndReference;
	}
	...
	public final String getActivityCodeByRnd(String codeRnd)
		...
		if (typeDoc == null) {
			serviceDispatcher.dispatch( new UnknownCodeRndEx(ResourceMessagesUtils.loadMessage(
